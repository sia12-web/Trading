...
- Desk is live-trading the US Range window (${midWindow} ${tzLabel}) — up to 2 probes @ 0.25% (entries within ±10 pts of US Range H / 50% mid / L). Morning OR30 does not lock this window.
...
`
    ...
    const deskCadence = tokyo
      ? `- Three ranges / up to 2 probes each @ 0.25% (session cap ≤ 3 fills total, win/loss/breakeven all count): Morning OR30 ${open}–${entryEnd} ${tzLabel} → US Range (prior NYC H/L) ${midWindow} ${tzLabel} → ${prepLabel} (levels update) → Tokyo IB ${lateWindow} ${tzLabel}. Next window unlocks when prior clock ends or probes are exhausted, but the session cap always wins even if a window still has spare probes. Entries only within ±10 pts of active range high, 50% mid, or low. Lunch ${lunch} ${tzLabel} is confirm-close only; unconfirmed books ride to cash-close flatten at ${close} ${tzLabel}.`
      : `- Three ranges / up to 2 probes each @ 0.25% (session cap ≤ 3 fills total, win/loss/breakeven all count): Morning OR30 ${open}–${entryEnd} ${tzLabel} → IB ${midWindow} ${tzLabel} → ${prepLabel} (levels update) → lunch-range ${lateWindow} ${tzLabel}. Next window unlocks when prior clock ends or probes are exhausted, but the session cap always wins even if a window still has spare probes. Entries only within ±10 pts of active range high, 50% mid, or low. Lunch ${lunch} ${tzLabel} is confirm-close only; unconfirmed books ride to cash-close flatten at ${close} ${tzLabel}.`
