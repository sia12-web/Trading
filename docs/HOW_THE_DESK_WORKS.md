...
4. **5-month anchored VWAP** — typical price `(H+L+C)/3`, standard deviation multipliers 1 / 2 / 3. Style: blue VWAP, green / olive / teal ±σ, fill between ±1σ. The VWAP series **stay on the price scale** (TradingView-style) so the bands do not vanish after the 5-month daily bars load. FRVP / POC stay on canvas and do not flatten the session.
