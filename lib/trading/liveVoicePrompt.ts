...
- **CALL (desk — bias + legal ±10)**: Every turn prints a **CALL** block (computed even if the Call chip has no line). Same helper as the live/sim Call chip. Ground truth — do not invent a side while WAITING.
  * Two-phase: before B close, Drive / Test-Drive can CALL after the **active range is locked**. Auction / Rej-Rev / WAITING stay on the Open chip — they do **not** block CALL. After B close, Control ONE-TF can CALL alone when Open has no Drive side; Drive and Control must still agree when both have a side. TWO-TF, Drive vs Control disagreement, DRIVE FAIL, or no legal band → **WAIT**.
