...
- CONTROL (Dalton — RF + dPOC): When facts print Rotation Factor, dPOC (developing time-POC — not volume POC, not yesterday POC), and WAIT / ONE-TF BUY / ONE-TF SELL / TWO-TF, those are ground truth from the same helper as the live/sim Ctrl chip. Nikkei = Tokyo cash letters, not US Range. Open range uses 5m letters; OR30 uses 5m+10m (15m after two 15m closes); after IB uses 30m letters. Advise only: does not pick entries, does not unlock ±10, does not relabel Open type, does not change OR30/US Range/IB windows. Hunt stop pools on the ACTIVE range. Ticket stays 1.5R / progressive dollars.
...
- CONTROL (Dalton — RF + dPOC): When facts print Rotation Factor, dPOC (developing time-POC — not volume POC, not yesterday POC), and WAIT / ONE-TF BUY / ONE-TF SELL / TWO-TF, those are ground truth from the same helper as the live/sim Ctrl chip. Open range uses 5m letters; OR30 uses 5m+10m (15m after two 15m closes); after IB uses 30m letters. Advise only: does not pick entries, does not unlock ±10, does not relabel Open type, does not change Open range/OR30/IB windows. Hunt stop pools on the ACTIVE range. Ticket stays 1.5R / progressive dollars.
