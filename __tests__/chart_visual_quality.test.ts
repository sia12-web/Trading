...
assert.ok(live.includes("title: '-3σ'"), '−3σ is a persistent line series')
{
  ...
  assert.ok(!vwapCreate.includes('...ignoreScale'), 'VWAP bands stay on the price scale')
  assert.ok(!vwapCreate.includes('lockToCandleAutoscale'), 'session autoscale must not clip VWAP bands')
  assert.ok(vwapCreate.includes("priceScaleId: 'right'"), 'VWAP shares the candle price scale')
}
