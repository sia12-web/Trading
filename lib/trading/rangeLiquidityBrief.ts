...
export function formatRangeLiquidityBriefForPrompt(
  ...
  const lines: string[] = [
    ...
    '',
    'RULE: Range H/L = retail BAIT (where retail enters). 50% mid is location only — never a legal entry. Desk EDGE ENTRY = stop pool JUST BEYOND H/L (±0.05–0.12% / wick pad). Prefer confluence with printed POC/HVN or AVWAP. Never return the exact range H/L as the edge-hunt entry print.',
    '',
  ]
  ...
  if (brief.active) {
    const usOnly = ` 50% mid ${brief.active.mid} is location only — not a legal entry.`
    lines.push(
      '',
