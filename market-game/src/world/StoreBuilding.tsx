...
import { fmtPx, fmtPx1, LADDER_TICKS, PRINT_HOLD_MS, rangeHeight } from '../game/auction'
...
export function StoreBuilding({ store }: { store: StoreDef }) {
  ...
  return (
    ...
    >
      ...
      />
      <ApronGauge
        p={apronGaugeLocal(store.range, shell.w, shell.d)}
        range={range}
        nodePx={price}
        color={store.accent}
      />
...
function RangeStoreys({
  ...
  const yLo = rangeHeight(range.lo, range.lo, range.hi, 0.28, 4.15)
  const yHi = rangeHeight(range.hi, range.lo, range.hi, 0.28, 4.15)
  const yVal = rangeHeight(range.val, range.lo, range.hi, 0.28, 4.15)
  const yVah = rangeHeight(range.vah, range.lo, range.hi, 0.28, 4.15)
  const yNode = rangeHeight(nodePx, range.lo, range.hi, 0.28, 4.15)
  const faces = cameraLadderLocals(rangeKind, width, depth)
  return (
    <>
      {faces.map((f) => (
        <Ladder key={f.face} p={f.p} r={f.r} wallW={f.w} yLo={yLo} yHi={yHi} yVal={yVal} yVah={yVah} yNode={yNode} color={color} />
      ))}
    </>
  )
}
...
function ApronGauge({
  ...
  const yLo = rangeHeight(range.lo, range.lo, range.hi, 0.32, 4.35)
  const yHi = rangeHeight(range.hi, range.lo, range.hi, 0.32, 4.35)
  const yVal = rangeHeight(range.val, range.lo, range.hi, 0.32, 4.35)
  const yVah = rangeHeight(range.vah, range.lo, range.hi, 0.32, 4.35)
  const yNode = rangeHeight(nodePx, range.lo, range.hi, 0.32, 4.35)
  return (
    <Ladder
      p={p}
      r={[0, Math.PI / 4, 0]}
...
function Ladder({
  ...
  const vaH = Math.max(1.15, yVah - yVal)
  const x = apron ? 0 : -wallW * 0.48
  const ticks = LADDER_TICKS
  const rungW = apron ? 1.28 : 1.05
  return (
    <group position={p} rotation={r}>
      <mesh position={[x, 2.35, 0.22]} castShadow>
        <boxGeometry args={[apron ? 0.42 : 0.34, 5.05, apron ? 0.42 : 0.32]} />
        <meshBasicMaterial color="#100c08" />
      </mesh>
      {Array.from({ length: ticks }, (_, i) => {
        const y = yLo + (i / (ticks - 1)) * (yHi - yLo)
        return (
          <mesh key={i} position={[x + rungW * 0.42, y, 0.42]} castShadow>
            <boxGeometry args={[rungW, 0.16, 0.22]} />
            <meshBasicMaterial color={i === 0 || i === ticks - 1 ? '#fff6d0' : '#f4ead8'} />
          </mesh>
        )
      })}
      <mesh position={[x + 0.62, (yVal + yVah) / 2, 0.48]} castShadow>
        <boxGeometry args={[apron ? 1.18 : 1.05, vaH, 0.38]} />
        <meshBasicMaterial color={color} />
      </mesh>
      <mesh position={[x + 0.72, yLo, 0.55]} castShadow>
        <boxGeometry args={[apron ? 1.42 : 1.22, 0.42, 0.32]} />
        <meshBasicMaterial color="#0a0806" />
      </mesh>
      <mesh position={[x + 0.72, yHi, 0.55]} castShadow>
        <boxGeometry args={[apron ? 1.42 : 1.22, 0.42, 0.32]} />
        <meshBasicMaterial color="#e8c04a" />
      </mesh>
      <mesh position={[x + 1.05, yNode, 0.62]} castShadow>
        <boxGeometry args={[apron ? 1.55 : 1.35, 0.24, 0.36]} />
...
function PorchTell({ stall, kind, z }: { stall: Stall; kind: StoreDef['kind']; z: number }) {
  const queued = stall.door > 0.45 && stall.occupancy > 0.22 && kind !== 'lvn'
  const settled = stall.fairToday && stall.timeOpportunity < 0.38 && stall.occupancy > 0.28
  const empty = kind === 'lvn' && !stall.clogged
  if (!queued && !settled && !empty) return null
  return (
    <group position={[0, 0, z]}>
      {[-1.25, 1.25].map((x) => (
...
function PrintBurst({ name, px }: { name: string; px: string }) {
  ...
  return (
    <group position={[0.35, 0.08, 4.55]}>
      {Array.from({ length: 48 }, (_, i) => (
        <mesh
          key={i}
          position={[((i % 6) - 2.5) * 0.78, 0.38 + Math.floor(i / 6) * 0.58, ((i % 4) - 1.5) * 0.52]}
          rotation={[0, (i % 4) * 0.08, 0]}
          castShadow
        >
          <boxGeometry args={[0.74, 0.52, 0.6]} />
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.03, 0.12]}>
        <ringGeometry args={[2.15, 3.35, 24]} />
        <meshBasicMaterial color="#c8d890" transparent opacity={0.94} toneMapped={false} />
      </mesh>
      <group position={[0.2, 2.55, 1.85]} rotation={[0, Math.PI / 4, 0]}>
        <mesh position={[0, 0, 0]} castShadow>
          <boxGeometry args={[5.65, 1.22, 0.2]} />
          <meshStandardMaterial color="#1a120c" />
        </mesh>
        <mesh position={[0, 0, 0.14]}>
          <planeGeometry args={[5.4, 1.02]} />
...
function Spire({
  ...
  return (
    <group>
      ...
      <mesh ref={cab} position={[0, 2.9, 2.35]} castShadow>
        <boxGeometry args={[2.65, 1.75, 1.55]} />
        <meshStandardMaterial
          color="#d4a046"
          metalness={0.42}
          roughness={0.48}
          emissive="#8a6a30"
          emissiveIntensity={printed ? 0.85 : 0.38}
        />
      </mesh>
      <mesh ref={needle} position={[2.15, 2.8, 2.15]} castShadow>
        <boxGeometry args={[0.58, 1.25, 0.95]} />
