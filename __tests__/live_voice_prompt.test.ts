...
assert(packed.includes('Call (desk bias + legal ±10)'), 'packed reminder has desk CALL')
assert(packed.includes('CALL is the read, not a fill'), 'packed reminder: locked book is not a fill')
assert(LIVE_VOICE_SYSTEM_PROMPT.includes('CALL bias/entry ±10'), 'Leo anti-hallucination lists CALL ±10')
assert(
  LIVE_VOICE_SYSTEM_PROMPT.includes('does **not** pick Level Finder entries') ||
    LIVE_VOICE_SYSTEM_PROMPT.includes('does not pick Level Finder entries'),
