...
## 6. Databento Live CME Gateway Sidecar Architecture

To eliminate pricing gaps during fast-moving New York and London opens without incurring round-trip Node.js HTTP overhead, TradePulse incorporates a dedicated Python live gateway daemon (`scripts/databento_live_sidecar.py`):

```
┌────────────────────────────────────────────────────────┐
│  CME Globex MDP 3.0 (Databento Live Feed)              │
└──────────────────────────┬─────────────────────────────┘
                           │ TCP Live Binary Stream
                           ▼
┌────────────────────────────────────────────────────────┐
│  Databento Live Gateway Daemon (127.0.0.1:8765)        │
│  • Subscribes: MNQ.FUT, MYM.FUT, MCL.FUT, NKD.FUT, MGC│
│  • Aggregates sub-second trade ticks & bid/ask spreads │
│  • Exposes local SSE (/stream) & snapshot (/snapshot)  │
└──────────────────────────┬─────────────────────────────┘
...
- **Ultra-Low Latency**: Directly pipes raw exchange trade events to the chart canvas with under 1ms local IPC latency.

---

## 7. Automated Level Reaction Grading Engine

Trade levels generated by the AI Playbook or historical auction analysis are graded continuously on every bar close:
- **Respected**: Price tested the zone and bounced in the anticipated direction with at least 1.0R continuation.
