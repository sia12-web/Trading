...
Databento hist is delayed; Yahoo CME stitches holes. If 5m slots are still missing near the tip (or the tip lags wall clock), `/api/trading/candles` **bypasses the 60s Databento cache and reprints** Databento + Yahoo (Databento + Yahoo load in parallel). The live chart also requests `?reprint=1` when it sees local gaps (45s cooldown), using the **active timeframe’s bar size** so 30m never looks like five missing 5m slots. Timeframe switches skip reprint until the held book’s spacing matches the new TF (and paint from cache when that TF was loaded before). Market tabs paint from an in-memory book cache and prefetch on hover / after first load. Weekend / Globex halt holes are intentional and are not reprinted as flat bars.
